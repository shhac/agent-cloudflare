...
`agent-cloudflare` is read-first. Current mutation commands are fenced behind dry-run or explicit confirmation.

Write commands must meet these requirements before they are documented in the skill:
...
agent-cloudflare dns update <zone> <record-id> --content target.example.com --dry-run
